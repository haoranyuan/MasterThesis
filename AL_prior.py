...
def control_measure(traj, ):
    ...
    for i in range(len(traj)):
        ...
        if traj[j][0] - traj[i][0] > 1:
            crash += 1
    score = -np.sum(np.absolute(traj[:, 0] - (SHAPE[0]-1)/2)) - 100 * crash
    return score
...
def reward_reconstruct(DIR, ITER, init_count=0):
    ...
    if (iter_count + 1) % DEMO_DELTA == 0 and (iter_count+1) != ITER:
        print('AL: update the demonstration at iter-'+str(iter_count))
...
if __name__ == "__main__":
    ...
    ITER = 20
    for _ in range(ITER):
        iter = reward_reconstruct(DIR, init_count=0, ITER=ITER)
