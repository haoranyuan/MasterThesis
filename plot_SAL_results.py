...
# DIR = 'AL_results/p_projection_good_07noise/p_projection_good_07noise'
DIR = 'AL_results/p_projection_good_09noise/p_projection_good_09noise'
# DIR = 'AL_results/p_projection_good/p_projection_good'
...
fea_e = np.zeros(shape=[REPEATS, ITER, 462], dtype=float)
dis_mixed = np.zeros(shape=[REPEATS, ITER])
dis_AL = np.zeros(shape=[REPEATS, ITER])
dis = np.zeros(shape=[REPEATS, ITER])

for i in range(REPEATS):
    MU_e = np.genfromtxt(DIR + str(i) + '/demo_history.csv', delimiter=',')[1:]
    ...
    dis_mixed[i, :] = [np.linalg.norm(f - fea_e[i, j, :]) for j, f in enumerate(mixed_fea[i, :, :])]
    dis_AL[i, :] = [np.linalg.norm(f - fea_e[i, j, :]) for j, f in enumerate(fea__[i, :, :])]
    dis[i, :] = [np.linalg.norm(f - fea_e[i, j, :]) for j, f in enumerate(fea[i, :, :])]
...
ax1.errorbar(np.arange(0, ITER), dis_mixed_aver, capsize=3, yerr=error_mixed, label='SAL')
ax1.errorbar(np.arange(0, ITER), dis_AL_aver, yerr=error_AL, capsize=3, label='SAL-Proj')
ax1.errorbar(np.arange(0, ITER), dis_aver, color='g', yerr=error, capsize=3, label='SAL-RL')
...
_, caps, bars = ax2.errorbar(np.arange(0, ITER), score_aver[2], capsize=3, yerr=score_error[0], label='SAL')

_, caps, bars = ax2.errorbar(np.arange(0, ITER), score_aver[0], color='g', capsize=3, yerr=score_error[0], label='SAL-RL')
